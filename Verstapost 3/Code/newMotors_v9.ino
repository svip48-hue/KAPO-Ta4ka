...
float  gyroZ       = 0;    // угловая скорость рысканья от AtomS3R IMU (°/с)
...
<div class="calib-section">
  ...
  <div class="calib-row">
    ...
    <input class="calib-input" type="number" id="cal-height" value="4.5" step="0.5" min="1" max="30">
    <span class="calib-unit">cm</span>
  </div>
  <div class="calib-row">
    <span class="calib-label">Kaamera nurk alla</span>
    <input class="calib-input" type="number" id="cal-angle" value="0" step="5" min="0" max="60">
...
// ── Калибровка камеры ───────────────────────────────────────
let calib = {
  height_cm:   4.5,   // высота камеры от пола
  angle_deg:   0,     // угол наклона вниз (0 = горизонтально)
...
function applyCalib() {
  calib.height_cm      = parseFloat(document.getElementById('cal-height').value)    || 4.5;
  calib.angle_deg      = parseFloat(document.getElementById('cal-angle').value)     || 0;
...
document.getElementById('cam-img').addEventListener('click', (e) => {
  ...
  const px_angle_h = (nx - 0.5) * calib.fov_h_deg;  // горизонтальный угол (°)
  const py_angle_v = (0.5 - ny) * calib.fov_v_deg;  // вертикальный угол (°)
  ...
  // Скорости бортов для поворота
  const base = currentSpeed;
  const turnFactor = Math.min(1.0, Math.abs(turn_deg) / 45);
  let lSpd, rSpd;
  ...
  if (Math.abs(turn_deg) > 3) {
    ...
    if (turn_deg > 0) {
      ...
      lSpd = Math.round(base * turnFactor);
      rSpd = 0;
    } else {
      // Влево
      lSpd = 0;
      rSpd = Math.round(base * turnFactor);
    }
