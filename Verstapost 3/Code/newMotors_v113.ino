...
float  gyroZ       = 0;    // угловая скорость рысканья от AtomS3R IMU (°/с), приходит раз в 150мс

// ── Коррекция курса по гироскопу ──────────────────────────
bool  gyroCorrectionEnabled = true;
float gyroKp           = 1.8;   // коэффициент коррекции (подбирается экспериментально)
...
// ── Пины DRI0044 #1 (левый борт) ───────────────────────────
#define R_PWM1  2   // D0 - левый мотор PWM
#define R_DIR1  3   // D1 - левый мотор DIR
#define R_PWM2  4   // D2 - правый мотор PWM (на левом драйвере)
#define R_DIR2  5   // D3 - правый мотор DIR (на левом драйвере)

// ── Пины DRI0044 #2 (правый борт) ──────────────────────────
#define L_PWM1  6   // D4 - левый мотор PWM (на правом драйвере)
#define L_DIR1  7   // D5 - левый мотор DIR (на правом драйвере)
#define L_PWM2  21  // D6 - правый мотор PWM (на правом драйвере)
#define L_DIR2  20  // D7 - правый мотор DIR (на правом драйвере)
...
// ── Коррекция курса по гироскопу (P-контроллер) ───────────
// gyroZ положительный = робот вращается по часовой стрелке (заносит вправо)
// Тогда нужно притормозить правый борт и/или ускорить левый.
// Данные gyroZ обновляются раз в ~150мс от AtomS3R, применяем каждый loop().
void correctCourse() {
  if (!gyroCorrectionEnabled) return;
  if (!isDriving) return;
  if (leftDir != FORWARD || rightDir != FORWARD) return;  // только прямое движение

  float correction = gyroKp * gyroZ;
  correction = constrain(correction, -60, 60);  // не давать коррекции зашкаливать
  lastCorrection = correction;

  int newL = constrain(baseLeftSpeed  - (int)correction, MIN_SPEED, MAX_SPEED);
  int newR = constrain(baseRightSpeed + (int)correction, MIN_SPEED, MAX_SPEED);

  // Обновляем только если изменение заметно — не дёргаем ШИМ зря
  if (abs(newL - leftSpeed) > 2 || abs(newR - rightSpeed) > 2) {
    leftSpeed  = newL;
    rightSpeed = newR;
    applyMotors();
  }
...
void applyCommand(const String& cmd) {
  ...
  autoStopped = false;
   ...
   // ---- НАЧАЛО ИЗМЕРЕНИЯ ЛАТЕНТНОСТИ (для LAT и LAT_MOTOR) ----
    uint32_t t_receive = micros();  // точное время получения
    bool measureLatency = (cmd == "LAT" || cmd == "LAT_MOTOR");
    if (measureLatency) {
  ...
  // lSpd/rSpd могут быть отрицательными — означает BACKWARD (для разворота на месте)
  else if (cmd.startsWith("DRIVE")) {
    int lSpd, rSpd, duration;
    if (sscanf(cmd.c_str(), "DRIVE %d %d %d", &lSpd, &rSpd, &duration) == 3) {
      lSpd     = constrain(lSpd, -MAX_SPEED, MAX_SPEED);
      rSpd     = constrain(rSpd, -MAX_SPEED, MAX_SPEED);
      duration = constrain(duration, 50, 3000);

      // Определяем направление по знаку скорости
      MotorDir lDir = (lSpd > 0) ? FORWARD : (lSpd < 0 ? BACKWARD : STOPPED);
      MotorDir rDir = (rSpd > 0) ? FORWARD : (rSpd < 0 ? BACKWARD : STOPPED);
      int lAbs = abs(lSpd);
      int rAbs = abs(rSpd);
      setMotors(lDir, rDir, lAbs, rAbs);

      // Сохраняем базовые скорости — коррекция курса будет отталкиваться от них
      // (используется только когда оба борта FORWARD, поэтому абсолютные значения ок)
      baseLeftSpeed  = lAbs;
      baseRightSpeed = rAbs;
    ...
    gyroCorrectionEnabled = true;
    Serial.println("Gyro correction ON");
  }
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
  ...
  .cam-box img{width:100%;height:100%;object-fit:fill;display:none}
  ...
  .calib-title{font-size:12px;color:var(--muted);font-weight:500;letter-spacing:.3px}
  .calib-row{display:flex;gap:8px;align-items:center}
  .calib-label{font-size:11px;color:var(--muted);width:130px;flex-shrink:0}
  .calib-input{
    flex:1;background:var(--bg);border:1px solid var(--border);
    border-radius:8px;color:var(--text);font-size:12px;
    padding:4px 8px;outline:none;text-align:center;
  }
  .calib-unit{font-size:11px;color:var(--muted);width:24px}
  .calib-btn{
    background:var(--accent);border:none;border-radius:8px;
    color:#fff;font-size:11px;padding:5px 12px;cursor:pointer;
    width:100%;margin-top:2px;
  }
  .calib-result{font-size:11px;color:var(--success);text-align:center;min-height:16px}
...
<!-- Калибровка камеры для click-to-drive -->
<div class="calib-section">
  <div class="calib-title">&#x1F4CF; Kalibreerimisparameetrid (klõpsa-ja-sõida)</div>
  <div class="calib-row">
    <span class="calib-label">Kaamera kõrgus</span>
    <input class="calib-input" type="number" id="cal-height" value="9" step="0.5" min="1" max="30">
    <span class="calib-unit">cm</span>
  </div>
  <div class="calib-row">
    <span class="calib-label">Kaamera nurk alla</span>
    <input class="calib-input" type="number" id="cal-angle" value="25" step="5" min="0" max="60">
    <span class="calib-unit">°</span>
  </div>
  <div class="calib-row">
    <span class="calib-label">Roboti kiirus</span>
    <input class="calib-input" type="number" id="cal-speed" value="10" step="1" min="1" max="50">
    <span class="calib-unit">cm/s</span>
  </div>
  <div class="calib-row">
    <span class="calib-label">Pöördekiirus</span>
    <input class="calib-input" type="number" id="cal-turnspeed" value="90" step="10" min="10" max="360">
    <span class="calib-unit">°/s</span>
  </div>
  <div class="calib-row">
    <span class="calib-label">Güroskoobi korrektsioon</span>
    <button class="cam-toggle on" id="gyro-toggle" onclick="toggleGyroCorr()" style="flex:1">Sees</button>
  </div>
...
// ── Калибровка камеры ───────────────────────────────────────
let calib = {
  height_cm:   9,     // высота камеры от пола
  angle_deg:   25,    // угол наклона вниз
  speed_cms:   10,    // скорость робота см/с
  turnspeed_degs: 90, // скорость поворота градус/с
  fov_h_deg:   60,    // горизонтальный FOV OV3660
  fov_v_deg:   45,    // вертикальный FOV OV3660
};

let gyroCorrOn = true;
function toggleGyroCorr() {
  gyroCorrOn = !gyroCorrOn;
  const btn = document.getElementById('gyro-toggle');
  btn.textContent = gyroCorrOn ? 'Sees' : 'Väljas';
  btn.classList.toggle('on', gyroCorrOn);
...
function applyCalib() {
  ...
  calib.speed_cms      = parseFloat(document.getElementById('cal-speed').value)     || 10;
  calib.turnspeed_degs = parseFloat(document.getElementById('cal-turnspeed').value) || 90;

  // Рассчитать видимый диапазон
  const angDown = calib.angle_deg + calib.fov_v_deg / 2;
  const angUp   = calib.angle_deg - calib.fov_v_deg / 2;
  const near = angDown > 0 ? (calib.height_cm / Math.tan(angDown * Math.PI/180)).toFixed(1) : '∞';
  const far  = angUp  > 0 ? (calib.height_cm / Math.tan(angUp   * Math.PI/180)).toFixed(1) : '∞';

  document.getElementById('calib-result').textContent =
    `Näeb põrandat: ${near}–${far} cm kaugusel ✓`;
  console.log('Calib applied:', calib);
}

// ── Click-to-drive (геометрический расчёт) ─────────────────
document.getElementById('cam-img').addEventListener('click', (e) => {
  if (!wsReady || !camOn) return;

  const rect = e.target.getBoundingClientRect();
  // Нормализованные координаты (0-1)
  const nx = (e.clientX - rect.left) / rect.width;   // 0=лево 1=право
  const ny = (e.clientY - rect.top)  / rect.height;  // 0=верх 1=низ

  // Угол пикселя относительно центра камеры
  const px_angle_h = (0.5 - nx) * calib.fov_h_deg;  // горизонтальный угол (°), инвертирован из-за поворота кадра
  // py_angle_v: положительный = клик НИЗ экрана = смотрим больше вниз к полу (близко)
  const py_angle_v = (ny - 0.5) * calib.fov_v_deg;  // вертикальный угол (°)

  // Угол луча к полу
  const ray_angle = calib.angle_deg + py_angle_v;

  let dist_cm, turn_deg;

  if (ray_angle <= 0) {
    // Луч уходит в горизонт или выше — далеко
    dist_cm = calib.speed_cms * 3.0;  // 3 секунды езды
  } else {
    // Расстояние через геометрию
    dist_cm = calib.height_cm / Math.tan(ray_angle * Math.PI / 180);
    dist_cm = Math.min(dist_cm, calib.speed_cms * 4.0);  // не более 4 сек
    dist_cm = Math.max(dist_cm, 3);                        // минимум 3 см
  }

  // Угол поворота
  turn_deg = px_angle_h;  // горизонтальный угол пикселя = угол поворота

  // Времена
  const turnTime_ms  = Math.abs(turn_deg) > 3
    ? Math.round(Math.abs(turn_deg) / calib.turnspeed_degs * 1000)
    : 0;
  const driveTime_ms = Math.round(dist_cm / calib.speed_cms * 1000);

  const base = currentSpeed;
  let lSpd, rSpd;

  if (Math.abs(turn_deg) > 3) {
    // ── Разворот на месте — ТА ЖЕ физика что у кнопок L/R ──
    // Постоянная скорость (не масштабируем по углу!) — время определяет угол.
    // turn_deg > 0 → клик слева на экране → как кнопка 'L': левый FORWARD, правый BACKWARD
    // turn_deg < 0 → клик справа на экране → как кнопка 'R': левый BACKWARD, правый FORWARD
    if (turn_deg > 0) {
      lSpd = base;
      rSpd = -base;
    } else {
    ...
    send(`DRIVE ${lSpd} ${rSpd} ${turnTime_ms}`);

    // Потом прямо
    setTimeout(() => {
      send(`DRIVE ${base} ${base} ${driveTime_ms}`);
    }, turnTime_ms + 50);
  } else {
    // Почти прямо — просто едем
    send(`DRIVE ${base} ${base} ${driveTime_ms}`);
  }

  // Визуальный отклик
  showClickDot(e.clientX - rect.left, e.clientY - rect.top, rect.width, rect.height);
  document.getElementById('cur-cmd').textContent =
    `klõps: ${dist_cm.toFixed(0)}cm, ${turn_deg.toFixed(0)}°`;
});
...
function updateSensorUI(d) {
  ...
  // Гироскоп Z (рысканье)
  if (d.gyro_z !== undefined) {
    const el = document.getElementById('s-gyro');
    el.textContent = d.gyro_z.toFixed(1);
...
void handleSensor() {
  ...
  if (doc.containsKey("gyro_z"))    gyroZ      = doc["gyro_z"].as<float>();
...
void loop() {
  ...
  wsServer.loop();
  updateSensors();
  correctCourse();   // коррекция курса по гироскопу — только во время DRIVE
